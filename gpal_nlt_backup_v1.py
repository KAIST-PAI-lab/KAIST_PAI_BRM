...
for trial_idx in range(n_trials):
    ...
    dot_size = 4 if not size_control_order[trial_idx] else calculate_dot_size(max_number_size=4, number=given_number, max_number=max_number)

    # Show the dots and get response from participant
    response = show_and_get_response(given_number, dot_size, visuals, max_number=max_number)
