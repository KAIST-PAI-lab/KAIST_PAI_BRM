...
## A helper function for constructing stimulus candidates
from gpalexp import sequence_with_interval
## Functions for plotting the figures
from gpalexp import plot_convergence, plot_GP, plot_selection_frequency
...
## We can create a sequence of stimulus candidates with sequence_with_interval() function.
## Then we chose a random candidate as a stimulus for the first trial.
stimulus_list=sequence_with_interval(5, 500, 5)
choice_index=np.random.choice(len(stimulus_list), 1)
initial_stimulus=stimulus_list[choice_index].item()
...
for trial_index in range(1, num_trials):
    ...
    optimal_design, gp_mean, gp_std, lml = gpal_optimize(gpr,                                 # A GP regressor object to be fitted.
...
figure_GP, axes_GP = plot_GP(gpr, results_df)
figure_GP.show()
figure_GP.savefig(figure_GP_path)

figure_freq, ax_freq = plot_selection_frequency(results_df)
figure_freq.show()
figure_freq.savefig(figure_frequency_path)

figure_conv, axes_conv, mse_values = plot_convergence(gpr, results_df)
figure_conv.show()
figure_conv.savefig(figure_convergence_path)
