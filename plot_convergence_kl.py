...
if __name__=="__main__":
    ...
    for idx, (kgs, cg) in enumerate(zip(kl_gpal_sum, counts_gpal)):
        if cg!=0:
            kl_gpal_mean[idx]=kgs/cg
    for idx, (kbs, cb) in enumerate(zip(kl_bd_sum, counts_bd)):
        if cb!=0:
            kl_bd_mean[idx]=kbs/cb
    ...
    # --- BD plot ---
    plt.plot(trials[2:], kl_mean[2:,1], color="purple", marker="o", linewidth=1, markersize=3, 
    ...
    plt.title("Mean Convergence Plot (Inliers)", fontsize=20, fontweight="bold")
